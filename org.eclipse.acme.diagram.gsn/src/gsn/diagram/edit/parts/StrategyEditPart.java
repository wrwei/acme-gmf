	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.STRATEGY_DIMENSION.height;
			bounds.width = DimensionUtil.STRATEGY_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureStrategyName();
		nameLabel.setText(ModelElementFeatureUtil.getName(strategy));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(bounds.width/5, 5, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureStrategyDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(strategy));
		Dimension descriptionDimension = DimensionUtil.getTextSize(descriptionLabel.getText(),
				descriptionLabel.getFont(), bounds.width * 3 / 5);
		getPrimaryShape().setConstraint(descriptionLabel,
				new Rectangle(bounds.width/5, 35, descriptionDimension.width, descriptionDimension.height));
	}
