	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.SOLUTION_DIMENSION.height;
			bounds.width = DimensionUtil.SOLUTION_DIMENSION.width;
		}
		
		int width = (int) Math.round(Math.sqrt(bounds.width * bounds.width / 2));
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureSolutionName();
		nameLabel.setText(ModelElementFeatureUtil.getName(solution));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle((bounds.width - width) / 2 + 5, (bounds.width - width) / 2 + 5,
				nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureSolutionDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(solution));
		Dimension descriptionDimension = DimensionUtil.getTextSize(descriptionLabel.getText(),
				descriptionLabel.getFont(), (int) Math.round(Math.sqrt(bounds.width * bounds.width / 2)));
		getPrimaryShape().getLayoutManager().setConstraint(descriptionLabel,
				new Rectangle((bounds.width - width) / 2 + 5, (bounds.width - width) / 2 + 35,
						descriptionDimension.width, descriptionDimension.height));
	}
