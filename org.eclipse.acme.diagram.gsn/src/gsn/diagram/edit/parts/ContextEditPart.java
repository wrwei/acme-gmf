	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.CONTEXT_DIMENSION.height;
			bounds.width = DimensionUtil.CONTEXT_DIMENSION.width;
		}
		
		int offset = (int) Math.round(bounds.width / 2 / 1.618 / 2);
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureContextName();
		nameLabel.setText(ModelElementFeatureUtil.getName(context));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle(offset, 5, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureContextDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(context));
		Dimension descriptionDimension = DimensionUtil.getTextSize(descriptionLabel.getText(),
				descriptionLabel.getFont(), bounds.width - offset * 2);
