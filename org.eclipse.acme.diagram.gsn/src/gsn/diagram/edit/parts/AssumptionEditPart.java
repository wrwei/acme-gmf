	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.ASSUMPTION_DIMENSION.height;
			bounds.width = DimensionUtil.ASSUMPTION_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureAssumptionName();
		nameLabel.setText(ModelElementFeatureUtil.getName(assumption));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle(bounds.width / 3 + 5, 5, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureAssumptionDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(assumption));
