	...
	*/
	...
	public class ContractModuleReferenceFigure extends ContractModuleReferenceShape {
		...
		public ContractModuleReferenceFigure() {
			this.setContractModuleReference((ContractModuleReference) resolveSemanticElement());
		...
		private void createContents() {
			...
			Dimension descriptionDimension = DimensionUtil.getTextSize(fFigureContractModuleRefDescription.getText(),
					fFigureContractModuleRefDescription.getFont(), DimensionUtil.CONTRACT_MODULE_DIMENSION.width - 10);
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.CONTRACT_MODULE_DIMENSION.height;
			bounds.width = DimensionUtil.CONTRACT_MODULE_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureContractModuleRefName();
		nameLabel.setText(ModelElementFeatureUtil.getName(contractModuleReference));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle(5, ContractModuleReferenceShape.TOP_RECT_HEIGHT + 5, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureContractModuleRefDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(contractModuleReference));
		...
		getPrimaryShape().getLayoutManager().setConstraint(descriptionLabel,
				new Rectangle(5, ContractModuleReferenceShape.TOP_RECT_HEIGHT + 35, descriptionDimension.width, descriptionDimension.height));
	}
