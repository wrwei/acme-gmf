	...
	*/
	...
	public class GoalFigure extends GoalShape {
		...
		private void createContents() {
			...
			Dimension descriptionDimension = DimensionUtil.getTextSize(fFigureGoalDescription.getText(),
					fFigureGoalDescription.getFont(), DimensionUtil.GOAL_DIMENSION.width);
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.GOAL_DIMENSION.height;
			bounds.width = DimensionUtil.GOAL_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureGoalName();
		...
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureGoalDescription();
