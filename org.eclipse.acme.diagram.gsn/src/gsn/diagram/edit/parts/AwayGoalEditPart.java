	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.AWAY_GOAL_DIMENSION.height;
			bounds.width = DimensionUtil.AWAY_GOAL_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureAwayGoalName();
		nameLabel.setText(ModelElementFeatureUtil.getName(awayGoal));
		...
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureAwayGoalDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(awayGoal));
		...
		WrappingLabel moduleLabel = getPrimaryShape().getFigureAwayGoalModule();
		Goal goal = (Goal) awayGoal.getCitedElement();
		...
		int y_offset = bounds.height - AwayGoalShape.MODULE_ICON_OFFSET
				- AwayGoalShape.MODULE_ICON_HEIGHT;
		int x_offset = AwayGoalShape.MODULE_ICON_OFFSET + AwayGoalShape.MODULE_ICON_WIDTH + 5;
