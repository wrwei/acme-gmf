	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.AWAY_SOLUTION_DIMENSION.height;
			bounds.width = DimensionUtil.AWAY_SOLUTION_DIMENSION.width;
		}
		...
		WrappingLabel nameLabel = getPrimaryShape().getFigureAwaySolutionName();
		nameLabel.setText(ModelElementFeatureUtil.getName(awaySolution));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle(offset + 5, offset, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureAwaySolutionDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(awaySolution));
		Dimension descriptionDimension = DimensionUtil.getTextSize(descriptionLabel.getText(),
				descriptionLabel.getFont(), bounds.width - offset * 2 - 10);
		getPrimaryShape().getLayoutManager().setConstraint(descriptionLabel,
				new Rectangle(offset + 5, offset + 35, descriptionDimension.width, descriptionDimension.height));
		
		WrappingLabel moduleLabel = getPrimaryShape().getFigureAwaySolutionModule();
		Solution solution = (Solution) awaySolution.getCitedElement();
		...
		int y_offset = bounds.height - AwaySolutionShape.MODULE_ICON_OFFSET
				- AwaySolutionShape.MODULE_ICON_HEIGHT;
		int x_offset = AwaySolutionShape.MODULE_ICON_OFFSET + AwaySolutionShape.MODULE_ICON_WIDTH + 5;
