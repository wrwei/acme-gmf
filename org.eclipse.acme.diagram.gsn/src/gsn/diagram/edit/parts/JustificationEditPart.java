	...
	*/
	...
	public class JustificationFigure extends JustificationShape {
		...
		private void createContents() {
			...
			this.add(fFigureJustificationName,
					new Rectangle(r.width / 3 + 5, 10, nameDimension.width, nameDimension.height));
	...
	private void adjustLabelPositions()
	{
		...
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.JUSTIFICATION_DIMENSION.height;
			bounds.width = DimensionUtil.JUSTIFICATION_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureJustificationName();
		nameLabel.setText(ModelElementFeatureUtil.getName(justification));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().getLayoutManager().setConstraint(nameLabel, new Rectangle(bounds.width / 3+5, 10, nameDimension.width, nameDimension.height));
		
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureJustificationDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(justification));
		Dimension descriptionDimension = DimensionUtil.getTextSize(descriptionLabel.getText(),
				descriptionLabel.getFont(), bounds.width*5/7 - 10);
		getPrimaryShape().getLayoutManager().setConstraint(descriptionLabel,
				new Rectangle(bounds.width / 7 + 5, 35, descriptionDimension.width, descriptionDimension.height));
	}
