...
//		installEditPolicy(EditPolicyRoles.OPEN_ROLE, new OpenDiagramEditPolicy()); // XXX need an SCR to runtime to have another abstract superclass that would let children add reasonable editpolicies
	...
	*/
	...
	{
		ArgumentPackage goal = (ArgumentPackage) resolveSemanticElement();

		Rectangle bounds = getFigure().getBounds().getCopy();
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.ARGUMENT_PACKAGE_DIMENSION.height;
			bounds.width = DimensionUtil.ARGUMENT_PACKAGE_DIMENSION.width;
		} 
		...
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureApDesc();
