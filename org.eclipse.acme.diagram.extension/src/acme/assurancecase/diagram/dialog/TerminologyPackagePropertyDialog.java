...
public class TerminologyPackagePropertyDialog extends ModelElementPropertyDialog {
	...
	private TreeViewer terminologyTreeViewer;
	...
	protected void createViewerGroup(Composite container) {
		...
		terminologyTreeViewer = new TreeViewer(groupContent);
		terminologyTreeViewer.setContentProvider(new TerminologyTreeContentProvider());
		terminologyTreeViewer.getTree().setHeaderVisible(true);
		terminologyTreeViewer.getTree().setLinesVisible(true);
		GridData treeViewLayout = new GridData(SWT.FILL, SWT.FILL, true, true, 1, 3);
		treeViewLayout.minimumHeight = 200;
		terminologyTreeViewer.getControl().setLayoutData(treeViewLayout);
		
		
		TreeViewerColumn viewerColumn0  = new TreeViewerColumn(terminologyTreeViewer, SWT.NONE);
		...
		TreeViewerColumn viewerColumn1  = new TreeViewerColumn(terminologyTreeViewer, SWT.NONE);
		...
		TreeViewerColumn viewerColumn2  = new TreeViewerColumn(terminologyTreeViewer, SWT.NONE);
		...
		fillContent();
		terminologyTreeViewer.setInput(content);
		
		GridLayoutFactory.fillDefaults().generateLayout(container);
		
		terminologyTreeViewer.addDoubleClickListener(new IDoubleClickListener() {
...
//		});
		...
		add_button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				Shell activeShell = Display.getDefault().getActiveShell();
				TerminologyAssetCreationDialog dialog = new TerminologyAssetCreationDialog(activeShell, modelElement);
				dialog.create();
				if (dialog.open() == Window.OK) {
...
//					terminologyPackage.getTerminologyElement().add(dialog.getCreatedTerminoloyAsset());
		...
		remove_button.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if (terminologyTreeViewer.getSelection() instanceof IStructuredSelection) {
					final IStructuredSelection sel = (IStructuredSelection)terminologyTreeViewer.getSelection();
...
//						terminologyPackage.getTerminologyElement().remove(asset);
	...
	public void focusElement(ModelElement modelElement) {
		terminologyTreeViewer.setSelection(new StructuredSelection(modelElement), true);
	}
	...
	private class TerminologyAdapter extends EContentAdapter {
		...
		public void notifyChanged(Notification notification) {
			fillContent();
			terminologyTreeViewer.refresh(true);
		}
