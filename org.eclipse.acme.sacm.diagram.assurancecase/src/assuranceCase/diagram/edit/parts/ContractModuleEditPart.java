...
//		installEditPolicy(EditPolicyRoles.OPEN_ROLE, new assuranceCase.diagram.edit.policies.OpenDiagramEditPolicy()); // XXX need an SCR to runtime to have another abstract superclass that would let children add reasonable editpolicies
	...
	*/
	...
	private void adjustLabelPositions()
	{
		...
		WrappingLabel nameLabel = getPrimaryShape().getFigureCmName();
		nameLabel.setText(ModelElementFeatureUtil.getName(contractModule));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(5, ContractModuleShape.TOP_RECT_HEIGHT + 5, nameDimension.width,
				nameDimension.height));
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureCmDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(contractModule));
		...
		getPrimaryShape().setConstraint(descriptionLabel,
				new Rectangle(5, ContractModuleShape.TOP_RECT_HEIGHT + 35,
						descriptionDimension.width, descriptionDimension.height));
	}
}
