				...
				if (request instanceof ChangeBoundsRequest) {
					...
					if (d.width > minimum.width && d.height > minimum.height) {
						...
						nodeFigure.setConstraint(nodeFigure.getFigureApDescription(),
								new Rectangle(5+ArtifactPackageShape.TOP_RECT_WIDTH_OFFSET, ArtifactPackageShape.TOP_RECT_HEIGHT + 35, descriptionDimension.width,
	...
	*/
	...
	public class ArtifactPackageFigure extends ArtifactPackageShape {
		...
		private void createContents() {
			...
			this.add(fFigureApName, new Rectangle(5 + ArtifactPackageShape.TOP_RECT_WIDTH_OFFSET, ArtifactPackageShape.TOP_RECT_HEIGHT + 5, nameDimension.width,
			...
			this.add(fFigureApDescription, new Rectangle(5 + ArtifactPackageShape.TOP_RECT_WIDTH_OFFSET, ArtifactPackageShape.TOP_RECT_HEIGHT + 35,
	...
	private void adjustLabelPositions()
	{
		...
		WrappingLabel nameLabel = getPrimaryShape().getFigureApName();
		nameLabel.setText(ModelElementFeatureUtil.getName(artifactPackage));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(5 + ArtifactPackageShape.TOP_RECT_WIDTH_OFFSET, ArtifactPackageShape.TOP_RECT_HEIGHT + 5, nameDimension.width,
				nameDimension.height));
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureApDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(artifactPackage));
		...
		getPrimaryShape().setConstraint(descriptionLabel,
				new Rectangle(5 + ArtifactPackageShape.TOP_RECT_WIDTH_OFFSET, ArtifactPackageShape.TOP_RECT_HEIGHT + 35,
						descriptionDimension.width, descriptionDimension.height));
	}
