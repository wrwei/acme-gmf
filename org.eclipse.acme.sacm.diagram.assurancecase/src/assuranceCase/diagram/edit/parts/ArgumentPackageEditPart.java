...
//		installEditPolicy(EditPolicyRoles.OPEN_ROLE, new assuranceCase.diagram.edit.policies.OpenDiagramEditPolicy()); // XXX need an SCR to runtime to have another abstract superclass that would let children add reasonable editpolicies
	...
	*/
	...
	{
		ArgumentPackage argumentPackage = (ArgumentPackage) resolveSemanticElement();

		Rectangle bounds = getFigure().getBounds().getCopy();
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureArgpName();
		nameLabel.setText(ModelElementFeatureUtil.getName(argumentPackage));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(5 +  ArgumentPackageShape.TOP_RECT_WIDTH_OFFSET, ArgumentPackageShape.TOP_RECT_HEIGHT + 5, nameDimension.width,
				nameDimension.height));
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureArgpDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(argumentPackage));
		...
		getPrimaryShape().setConstraint(descriptionLabel,
				new Rectangle(5+ArgumentPackageShape.TOP_RECT_WIDTH_OFFSET, ArgumentPackageShape.TOP_RECT_HEIGHT + 35,
						descriptionDimension.width, descriptionDimension.height));
	}
