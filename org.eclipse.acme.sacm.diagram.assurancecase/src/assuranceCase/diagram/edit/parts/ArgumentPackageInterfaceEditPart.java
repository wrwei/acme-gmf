...
//		installEditPolicy(EditPolicyRoles.OPEN_ROLE, new assuranceCase.diagram.edit.policies.OpenDiagramEditPolicy()); // XXX need an SCR to runtime to have another abstract superclass that would let children add reasonable editpolicies
	...
	*/
	...
	{
		ArgumentPackageInterface argumentPackageInterface = (ArgumentPackageInterface) resolveSemanticElement();

		Rectangle bounds = getFigure().getBounds().getCopy();
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureArgpiName();
		nameLabel.setText(ModelElementFeatureUtil.getName(argumentPackageInterface));
		Dimension nameDimension = DimensionUtil.getTextSize(nameLabel.getText(), nameLabel.getFont());
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(5+ArgumentPackageInterfaceShape.TOP_RECT_WIDTH_OFFSET, ArgumentPackageInterfaceShape.TOP_RECT_HEIGHT + 5,
				nameDimension.width, nameDimension.height));
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureArgpiDescription();
		descriptionLabel.setText(ModelElementFeatureUtil.getDescription(argumentPackageInterface));
		...
		getPrimaryShape().setConstraint(descriptionLabel,
				new Rectangle(5+ArgumentPackageInterfaceShape.TOP_RECT_WIDTH_OFFSET, ArgumentPackageInterfaceShape.TOP_RECT_HEIGHT + 35,
						descriptionDimension.width, descriptionDimension.height));
	}
