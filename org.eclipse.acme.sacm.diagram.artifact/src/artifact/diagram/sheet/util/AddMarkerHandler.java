package artifact.diagram.sheet.util;
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection != null && selection instanceof IStructuredSelection) {
			...
			if (obj instanceof ArtifactEditPart) {
				ArtifactEditPart editPart = (ArtifactEditPart) obj;
				try {
					...
					if (dialog.open() == Window.OK) {
	        			...
	        			diagnostic.add(new BasicDiagnostic(severity, "org.eclipse.acme.sacm.diagram.artifact", 0, "" + message, new Object[] {editPart.resolveSemanticElement()}));

						ValidateAction.createMarkers(file, diagnostic, (ArtifactPackageCanvasEditPart)editPart.getParent());
						
						ValidationCentre validationCentre = ValidationCentre.getInstance();
						System.out.println(validationCentre.getErrors());
						for(ValidationError error: validationCentre.getErrors(editPart.resolveSemanticElement())) {
							ValidateAction.createMarkers(error.getFile(), error.getDiagnostic(), (ArtifactPackageCanvasEditPart)editPart.getParent());
						}
					}
				} catch (Exception e) {
