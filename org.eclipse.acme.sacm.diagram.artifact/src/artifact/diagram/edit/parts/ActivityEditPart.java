...
public class ActivityEditPart extends ShapeNodeEditPart {
	...
	*/
	...
	{
		Activity artifact = (Activity) resolveSemanticElement();

		Rectangle bounds = getFigure().getBounds().getCopy();
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.ACTIVITY_DIMENSION.height;
			bounds.width = DimensionUtil.ACTIVITY_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureActivityName();
		...
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureActivityDescription();
