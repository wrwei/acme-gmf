...
public class ResourceEditPart extends ShapeNodeEditPart {
	...
	*/
	...
	public class ResourceFigure extends ResourceShape {
		...
		public ResourceFigure() {
			...
			this.setMinimumSize(DimensionUtil.getMinimumDimension((ModelElement) resolveSemanticElement(),
					DimensionUtil.RESOURCE_DIMENSION));
			createContents();
		}
		...
		private void createContents() {
			...
			this.add(fFigureResourceName, new Rectangle(5, DimensionUtil.RESOURCE_DIMENSION.height/6, 
			...
			this.add(fFigureResourceDescription, new Rectangle(5, DimensionUtil.RESOURCE_DIMENSION.height/6 + 35, 
	...
	{
		Resource artifactPackage = (Resource) resolveSemanticElement();

		Rectangle bounds = getFigure().getBounds().getCopy();
		if (bounds.height == 0 && bounds.width == 0) {
			bounds.height = DimensionUtil.RESOURCE_DIMENSION.height;
			bounds.width = DimensionUtil.RESOURCE_DIMENSION.width;
		}
		
		WrappingLabel nameLabel = getPrimaryShape().getFigureResourceName();
		...
		getPrimaryShape().setConstraint(nameLabel, new Rectangle(5, bounds.height/6, nameDimension.width,
				nameDimension.height));
		
		WrappingLabel descriptionLabel = getPrimaryShape().getFigureResourceDescription();
		...
		getPrimaryShape().setConstraint(descriptionLabel, 
				new Rectangle(5, bounds.height/6+35,
						descriptionDimension.width, descriptionDimension.height));
	}
